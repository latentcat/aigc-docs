...
- 定论：根据 Github 中的 discuss ，以及 Reddit 上的超长性能贴讨论，较为一致的看法是新版UI+新版torch可以替代旧版的手动cudnn方案。**重要的是，终于可以作用到20,30系显卡了！**下文为对该方案的逐步操作[帖子链接](https://www.reddit.com/r/StableDiffusion/comments/y71q5k/comment/jcm67lu/?utm_source=share&utm_medium=web2x&context=3)![截图](/assets/webui-config/5.jpg)
  ...
  ![pip](/assets/webui-config/6.jpg)

  下载较慢的话可以尝试自行换 pip 源，等待安装，这可能需要下载10分钟

  2. **修改 webui-user.bat 配置**
  ...
  前往 [CUDA 下载](https://developer.nvidia.com/cuda-toolkit-archive)下载 11.8 版本，前往 [CUDNN 下载](https://developer.download.nvidia.com/compute/redist/cudnn/v8.8.0/local_installers/11.8/)下载对应你系统的版本，8.8.0的cudnn。

  或者通过[百度网盘地址](https://pan.baidu.com/s/1nf-Z2h7DbOVhrDJ28tZGhg?pwd=b90s)下载，但是可能会更新较慢

  4. **现在，让我们启动 web UI，查看底部信息**

  ![版本](/assets/webui-config/7.jpg)

  如果显示的是 torch2.0+cu118，就说明成功了，这个时候再次去 System Info 面板下跑下测试吧~

- 问题：

    开启 --opt-sdp-attention 后会提高显存的需求，你会发现之前hires可以开启的分辨率，现在会爆显存，你可以配合分块 VAE 插件来解决这个问题。
...
- Tip 2 : 如果你是已经安装了 PyTorch 1.3 的存储库，可以修改 webui-user.bat 包含以下内容：
    ...
    ![user截图](/assets/webui-config/8.jpg)
